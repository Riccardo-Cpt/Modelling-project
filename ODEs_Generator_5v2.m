function odes = ODEs_Generator_5v2(t,populations,parameters,treatment)
    %ODEs
    odes = zeros(1, 7);
    ...
    if treatment == string('chemotherapy') || treatment == string('chemotherapy + ALDH-high T-Cell') || treatment == string('chemotherapy + ALDH-low T-Cell')
        if (t >= 20 && t <= 25) || (t >= 27 && t <= 32)
            odes(7) = 6000 -eC*C;
        end
    end
    
    if treatment == string('ALDH-high T-Cell') || treatment == string('chemotherapy + ALDH-high T-Cell')
        if (t >= 20 && t <= 21) || (t >= 27 && t <= 28)
            odes(3) = 10^6 - deltaTS*Ts;
        end
    end
    
    if treatment == string('ALDH-low T-Cell') || treatment == string('chemotherapy + ALDH-low T-Cell')
        if (t >= 20 && t <= 21) || (t >= 27 && t <= 28)
            odes(4) = 10^6 - deltaTP*Tp;
        end
    end
